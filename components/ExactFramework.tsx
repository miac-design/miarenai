    ...
    <section className="relative bg-navy py-10 md:py-16 overflow-hidden">
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-6">
        <div className="text-center mb-10">
          ...
          <p className="mt-3 text-warm-gray">Five signals. One audit. Complete clarity.</p>
        </div>
        ...
        <div className="bg-[rgba(20,30,48,0.6)] border border-white/10 rounded-xl p-7 md:p-9">
          <p className="text-xs font-mono tracking-wider text-teal mb-3">SIGNAL {active + 1} OF 5</p>
          <h3 className="text-xl md:text-2xl font-bold text-white mb-3">{signals[active].name}</h3>
          <p className="text-gray-400 leading-relaxed mb-5">{signals[active].description}</p>
          <div className="teal-border-left">
            <p className="text-teal text-sm italic">{signals[active].insight}</p>
          </div>
        </div>

        <div className="flex justify-center gap-2 mt-5">
          {signals.map((_, i) => (
            <button key={i} onClick={() => setActive(i)} className={`h-1.5 rounded-full transition-all ${i === active ? "w-6 bg-teal" : "w-1.5 bg-white/30"}`} aria-label={`Signal ${i + 1}`} />
          ))}
        </div>
